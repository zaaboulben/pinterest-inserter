...
export async function GET(req: NextRequest) {
  ...
  const  ACCESS_TOKEN= cookieStore.get('pinterest_access_token')
    ...
    const responce = await axios.get('https://api.pinterest.com/v5/pins', {
...
return NextResponse.json({responce: responce.data}, { status: 200 })
    ...
    console.error('Failed to fetch pins', error.response || error.message);
    return NextResponse.json({ error: 'Failed to fetch pins', details: error.response?.data || error.message }, { status: 500 });
}
