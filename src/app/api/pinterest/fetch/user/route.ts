...
export async function GET(res:NextResponse, req:NextRequest){
  ...
  const  ACCESS_TOKEN= cookieStore.get('pinterest_access_token')
    ...
    const responce = await axios.get('https://api.pinterest.com/v5/user_account', {
...
return NextResponse.json({responce: responce.data}, { status: 200 })
    ...
    console.error('Failed to fetch user_account', error.response || error.message);
    return NextResponse.json({ error: 'Failed to fetch user_account', details: error.response?.data || error.message }, { status: 500 });
}
